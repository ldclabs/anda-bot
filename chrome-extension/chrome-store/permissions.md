...
4. `cookies`：读取、设置或删除指定网页的 Cookie，用于会话排查、登录状态辅助和网页任务执行。
5. `debugger`：通过 Chrome 调试协议执行更稳定的页面操作，例如原生点击与按键、截图、导出 PDF、读取无障碍树，以及等待网络空闲。
6. `downloads`：发起、查询、取消和打开下载任务，用于处理网页导出文件和下载结果。
7. `scripting`：在用户授权的页面中注入脚本，用于读取页面内容或执行点击、填写等浏览器操作。
8. `sidePanel`：提供 Chrome 侧边栏聊天界面，让用户在浏览网页时直接与 Anda 交互。
9. `storage`：在本地保存 Gateway URL、Bearer token、session id 和用户设置，以便自动连接本机服务。
10. `tabs`：列出、打开、切换和导航标签页，以支持跨网页研究、整理和网页任务执行。
11. `tts`：在用户需要时朗读助手回复，提供语音播放能力。
12. `webNavigation`：监听页面跳转和加载完成状态，帮助在打开、切换或导航标签页后更新页面上下文并等待页面就绪。
13. 主机权限：允许扩展在用户请求的网页上读取内容并执行浏览器动作，使本机智能体能处理不同网站上的任务。
...
4. `cookies`: Reads, sets, and deletes cookies for user-requested pages to help with session troubleshooting, sign-in related flows, and web tasks.
5. `debugger`: Uses the Chrome DevTools protocol for more reliable page actions such as native clicks and key presses, screenshots, PDF export, accessibility tree inspection, and waiting for network idle.
6. `downloads`: Starts, lists, cancels, and opens downloads so Anda can handle files produced by web tasks.
7. `scripting`: Injects scripts into user-authorized pages to read page content or perform browser actions such as clicking and filling fields.
8. `sidePanel`: Provides the Chrome side-panel chat interface so users can talk to Anda while browsing.
9. `storage`: Stores the Gateway URL, Bearer token, session id, and user settings locally so the extension can reconnect to the local service.
10. `tabs`: Lists, opens, switches, and navigates tabs to support multi-page research, organization, and browser-based tasks.
