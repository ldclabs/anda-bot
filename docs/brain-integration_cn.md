...
删除和误记修复会在受理前校验预览；过期或已丢弃的预览须重新审阅，删除还会检查完整原生范围是否变化。已受理的操作保留原操作身份用于恢复。误记修复完成后会显式清空 Bot Notes，即使 Brain 的产品 epoch 没有变化。Formation 重试会复用首次尝试保存的来源时间，因此 runner 传入较晚时间也不会改变暂存来源和输入摘要。
