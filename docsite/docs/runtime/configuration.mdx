...
| `keys/` | Legacy or explicitly exported signing-key files. New identity private keys are stored in the OS secure credential store by default. |
...
Anda local state, memory graph, conversations, channel state, cron state, logs, and workspace data are stored on your machine by default. Identity private keys are stored in the OS secure credential store by default, with explicit file keys under `~/.anda/keys/`.
