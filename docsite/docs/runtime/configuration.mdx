...
~/.anda/
  ...
  channels/
  db/
  keys/ # legacy, explicit, or Secret Service fallback keys
  logs/
  bundled-skills/
  sandbox/
  skills/
...
| `keys/` | Legacy, explicitly exported, or Linux Secret Service fallback signing-key files. New identity private keys are stored in the OS secure credential store by default when it is available. |
...
Anda local state, memory graph, conversations, channel state, cron state, logs, and workspace data are stored on your machine by default. Identity private keys are stored in the OS secure credential store by default, with explicit file keys and Linux Secret Service fallback keys under `~/.anda/keys/`.
