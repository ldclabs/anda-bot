...
One daemon can serve multiple trusted users through the same Anda agent. Add each user's Ed25519 public key under top-level `users`, then set a channel entry's `user` to that id. If `user` is omitted, the channel uses the local owner from `~/.anda/keys/user.key`.
