...
The release install scripts register Anda to start when the current user logs in and try to start the daemon immediately. For PowerShell, add `-NoAutostart` or `-NoStart` to opt out; for the shell installer, set `ANDA_NO_AUTOSTART=1` or `ANDA_NO_START=1`.
