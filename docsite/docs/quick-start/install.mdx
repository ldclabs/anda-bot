...
opens a GUI setup wizard for provider/API key/model, and checks for downloaded
updates that can be installed with a restart prompt.
...
The macOS shell installer also installs the menu bar launcher, registers it at login, and starts it immediately; the launcher starts the daemon after setup, can check for updates from the menu bar, and prompts to install and restart after an update is downloaded. Linux and PowerShell script installs register daemon autostart directly. For PowerShell, add `-NoAutostart` or `-NoStart` to opt out; for the shell installer, set `ANDA_NO_AUTOSTART=1` or `ANDA_NO_START=1`.
