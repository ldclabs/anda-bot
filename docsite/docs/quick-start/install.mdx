...
The Windows installer registers Anda to start when the current user logs in and tries to start the daemon immediately. Add `-NoAutostart` or `-NoStart` to opt out.
