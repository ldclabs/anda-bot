...
~/.anda/
  ...
  channels/
  db/
  keys/ # устаревшие, явные или fallback-ключи Secret Service
  logs/
  bundled-skills/
  sandbox/
  skills/
...
`db/` хранит память, беседы, состояние каналов и cron. `keys/` хранит устаревшие, явно экспортированные файловые ключи или fallback-ключи Linux Secret Service; новые приватные ключи идентичности по умолчанию сохраняются в защищенном хранилище учетных данных ОС, когда оно доступно. `bundled-skills/` содержит skills, управляемые обновлениями Anda, а `skills/` содержит личные skills, созданные, клонированные или измененные пользователем. `skills-manifest.json` хранит локальное состояние включения, отключения и перезагрузки; `skill-backups/` и `skill-trash/` хранят резервные копии перед редактированием и удаленные личные skills. `workspace/` — рабочая область для shell и файлов.

## Приватность

Локальные данные остаются на вашей машине; приватные ключи идентичности по умолчанию сохраняются в защищенном хранилище учетных данных ОС, а явно созданные файловые ключи и fallback-ключи Linux Secret Service находятся под `~/.anda/keys/`.
...
Провайдеры моделей могут получать prompts, резюме и запросы памяти. Выбирайте провайдеры, endpoints и proxy по требованиям приватности.
