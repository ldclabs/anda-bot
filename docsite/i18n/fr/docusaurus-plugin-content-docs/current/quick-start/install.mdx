...
Les installateurs de release enregistrent Anda au démarrage de session de l'utilisateur actuel et tentent de lancer le daemon immédiatement. Avec PowerShell, ajoutez `-NoAutostart` ou `-NoStart`; avec l'installateur shell, utilisez `ANDA_NO_AUTOSTART=1` ou `ANDA_NO_START=1`.
