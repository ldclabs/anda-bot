...
~/.anda/
  ...
  channels/
  db/
  keys/ # clés héritées, explicites ou de secours (Secret Service)
  logs/
  bundled-skills/
  sandbox/
  skills/
...
| `keys/` | Fichiers de clés de signature hérités, exportés explicitement ou de secours pour Linux Secret Service. Les nouvelles clés privées d'identité sont stockées par défaut dans le stockage d'informations d'identification sécurisé du système d'exploitation lorsqu'il est disponible. |
...
Les données d'état locales d'Anda, le graphe de mémoire, les conversations, l'état des canaux, l'état des tâches planifiées, les journaux et les données de l'espace de travail sont stockés par défaut sur votre machine. Les clés privées d'identité sont stockées par défaut dans le stockage d'informations d'identification sécurisé du système d'exploitation, avec des fichiers de clés explicites et des clés de secours Linux Secret Service sous `~/.anda/keys/`.

Sous Linux, si aucun fournisseur de Secret Service n'est disponible ou déverrouillé, Anda utilise par défaut les fichiers de clé privée sous `~/.anda/keys/` et affiche/enregistre un avertissement. Pour utiliser le Secret Service, démarrez et déverrouillez un fournisseur dans une session D-Bus utilisateur (par exemple, `gnome-keyring-daemon --start --components=secrets`), assurez-vous que `DBUS_SESSION_BUS_ADDRESS` est défini pour le processus Anda, puis redémarrez Anda. Les utilisateurs de KDE peuvent déverrouiller KWallet à la place.
