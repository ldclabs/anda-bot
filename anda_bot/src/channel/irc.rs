...
impl IrcChannel {
    ...
    async fn send_raw(writer: &mut WriteHalf, line: &str) -> Result<(), BoxError> {
...
impl Channel for IrcChannel {
    ...
    async fn listen(
        ...
        Self::send_raw(&mut writer, &format!("USER {} 0 * :anda", self.username)).await?;
        ...
        loop {
            ...
            match msg.command.as_str() {
                ...
                "CAP"
                    if sasl_pending && msg.params.iter().any(|p| p.contains("sasl")) => {
                        if msg.params.iter().any(|p| p.contains("ACK")) {
                            ...
                            log::warn!("IRC server does not support SASL, continuing without it");
                            sasl_pending = false;
                            let mut guard = self.writer.lock().await;
                            if let Some(ref mut w) = *guard {
                ...
                "904" | "905" | "906" | "907" => {
                    log::warn!("IRC SASL authentication failed ({})", msg.command);
                    sasl_pending = false;
                    let mut guard = self.writer.lock().await;
                    if let Some(ref mut w) = *guard {
                        Self::send_raw(w, "CAP END").await?;
                ...
                "001" => {
                    registered = true;
                    log::info!("IRC registered as {}", current_nick);
                ...
                "433" => {
                    ...
                    log::warn!("IRC nickname {current_nick} is in use, trying {alt}");
                    let mut guard = self.writer.lock().await;
                    if let Some(ref mut w) = *guard {
