...
- Help users start with `anda memory guide`, `/memory`, or the browser Memory workspace. Ordinary memory needs no inbox configuration or MIB. Precise record changes require a reviewed product preview; a natural-language correction only submits new information. Respect the immutable host memory mode; never imply it makes files, chats or provider processing incognito.
