...
Source builds use the locked registry releases of Brain (0.12.1), Nexus (0.13.4), DB/KIP and Core/Engine, keeping one DB/core type identity. The commented sibling Brain patch is an opt-in local development override. Brain HTTP accepts `command` or `operations` application arguments (no `kip` field) and returns KIP 2.0 envelopes; ordinary Bot tool responses retain their existing `result`/`error` format.
